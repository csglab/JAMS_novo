...
pos_to_wiggle <- function( pdwn_coeffs, shifting_pos = 5, inf_pct = 0.3 ){
  ...
  mean_end <- mean( motif$pct[ ( nrow(motif)-shifting_pos ):nrow(motif)] )
  
  if((mean_start > inf_pct)&(mean_end < inf_pct)){shift_pos <- -shifting_pos}
  if((mean_start < inf_pct)&(mean_end > inf_pct)){shift_pos <- +shifting_pos}
  
  if((mean_start > inf_pct)&(mean_end > inf_pct)){shift_pos <- "expand"}
  
  if((mean_start < inf_pct)&(mean_end < inf_pct)){shift_pos <- 0}
